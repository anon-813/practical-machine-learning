...
The Random Forest Model is our preferred model.  And we'll make output text files with our answers to the quiz off of the original `raw_test` data.

```{r warning=FALSE, error=FALSE, message=FALSE}
pred_rf2 <- predict(model_rf, raw_test, type = "class")

make_quiz_files = function(x){
  n = length(x)
  for(i in 1:n){
